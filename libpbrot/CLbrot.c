...
uint16_t* normalizeCLGrid(CLbucket_t* grid, int gridSize) {
	...
	uint16_t* outGrid = (uint16_t*)CoTaskMemAlloc(sizeof(uint16_t) * gridSize * gridSize);

	for (i = 0; i < gridSize; i++) {
		for (j = 0; j < gridSize; j++) {
			uint16_t val = ((double)grid[j + gridSize * i] / max) * 0xFFFF;	// the maximum value of a uint16
...
extern __declspec(dllexport) uint16_t* RunCLbrot(char* kern, uint8_t deviceNo, uint32_t gridSize, uint32_t maxIterations,
												uint32_t minIterations, uint32_t supersampling, CLfraction_t gridRange,
												CLfraction_t maxOrbit) {
	uint16_t* out = CoTaskMemAlloc(sizeof(uint16_t) * gridSize * gridSize);
	uint64_t i;
	for (i = 0; i < gridSize * gridSize; i++) {
		out[i] = i % 0xffff;
	}
	writeImage(gridSize, gridSize, out);
	return out;
	sclHard hardware;
	sclSoft software;
	...
	software = sclGetCLSoftware(kern, "buddhabrot", hardware);
	clock_t start = clock();
	...
	uint16_t* normalized = normalizeCLGrid(grid, gridSize);
	free(grid);
	return normalized;
	/*clock_t norm = clock();
